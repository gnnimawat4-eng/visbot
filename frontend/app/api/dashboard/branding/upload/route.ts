...
export async function POST(req: NextRequest) {
  ...
  // Optional override — used by owner console to upload for any company
  const overrideId = formData.get('company_id') as string | null

  const resolved = await resolveCompanyId()
  const companyId = overrideId ?? resolved
